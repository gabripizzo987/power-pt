...
import { FaTrash, FaSignOutAlt } from "react-icons/fa"; // Importa l'icona di eliminazione e logout
...
const DashboardPalestra = () => {
  ...
  const handleLogout = () => {
    ...
    navigate("/login-palestra"); // Reindirizza alla pagina di login
  };
...
const handleRemoveTrainer = async (trainerId) => {
  try {
    ...
    if (response.status === 200) {
      // Se la rimozione è riuscita, aggiorna lo stato locale
      setPersonalTrainers(prev => prev.filter(t => t._id !== trainerId));
      setRemovedTrainers(prev => [...prev, { ...response.data }]);
...
// Rimuove un personal trainer approvato
const handleRemoveApprovedTrainer = async (trainerId) => {
  if (window.confirm("Sei sicuro di voler rimuovere questo trainer?")) {
    try {
      const token = localStorage.getItem("token");
      if (!token) {
        setMessage("Token non trovato. Effettua il login.");
      ...
      // Chiamata API per rimuovere il trainer approvato
      const response = await axios.delete(
        `http://localhost:3000/api/remove-trainer/${trainerId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      if (response.status === 200) {
        // Se la rimozione è riuscita, aggiorna la lista dei trainer approvati
      ...
      <div>
        <nav>
          <ul className="space-y-4">
            ...
            <li className="text-red-500 hover:text-red-700 cursor-pointer" onClick={handleLogout}>
            ...
            {personalTrainers.filter(trainer => trainer.approved && !trainer.isDeleted).length > 0 ? (
              personalTrainers.filter(trainer => trainer.approved && !trainer.isDeleted).map((trainer) => (
                <tr key={trainer._id} className="border-b">
                  ...
                  <td className="p-4">{trainer.esperienza} anni</td>
                  <td className="p-4">
                    <button onClick={() => handleRemoveApprovedTrainer(trainer._id)} className="bg-red-500 text-white py-1 px-3 rounded flex items-center">
              ...
              <tr>
                <td colSpan="4" className="text-center p-4 text-gray-500">Nessun trainer approvato</td>
              </tr>
            )}
          </tbody>
        </table>
